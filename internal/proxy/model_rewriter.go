	...
	if bytes.Contains(body, []byte(`"request"`)) {
		...
		if err := json.Unmarshal(newBody, &doc); err == nil {
			if req, ok := doc["request"].(map[string]interface{}); ok {
				...
				// Strip thinking blocks from past conversation turns when targeting Claude/GPT.
				// Anthropic strictly validates HMAC cryptographic signatures on all 'thinking' blocks in conversation history.
				// Past turns generated by Gemini lack Anthropic's signature, causing HTTP 400 "Invalid signature in thinking block".
				// Anthropic does not require past thoughts to continue conversation context.
				if targetCat == CategoryClaudeGPT {
								...
								if parts, ok := cMap["parts"].([]interface{}); ok {
									...
									for _, p := range parts {
										if pMap, ok := p.(map[string]interface{}); ok {
											...
											if _, hasSig := pMap["thoughtSignature"]; hasSig {
												delete(pMap, "thoughtSignature")
												changed = true
											}
										}
